...
public class AccountView extends JFrameView {
	...
	public void initComboBox() {
		...
		for(int x = 0; x < titles.length; x++) {
			...
			// create a us window for the current account
			usBox usTemp = new usBox(((AccountModel)getModel()), ((AccountController)getController()), titles[x]);
			usTemp.setVisible(false);
			usList.add(usTemp);
			
			// do the same for euro
			euroBox euroTemp = new euroBox(((AccountModel)getModel()), ((AccountController)getController()), titles[x]);
			euroTemp.setVisible(false);
			euroList.add(euroTemp);
			
			// do the same for yuan
			yuanBox yuanTemp = new yuanBox(((AccountModel)getModel()), ((AccountController)getController()), titles[x]);
			yuanTemp.setVisible(false);
			yuanList.add(yuanTemp);
